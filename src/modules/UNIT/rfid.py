...
from modules.UNIT.mfrc522 import MFRC522
import ubinascii
...
class RFIDUnit(MFRC522):
    ...
    def read(self, block_addr, key):
        ...
        """
        buffer = bytearray(16)
        self.pcd_authenticate(0x60, block_addr, ubinascii.unhexlify(key), self._uid)
        print(self.mifare_read(block_addr, buffer))
        if self.mifare_read(block_addr, buffer) == self.STATUS_OK:
            return buffer
        else:
            return None

    def write(self, block_addr, buffer, key) -> int:
        """
        note:
